 ...
 * */
	...
	private List<Operate> operateList = new ArrayList<Operate>();  //保存生成的随机操作序列，有并发控制和无并发控制使用同?操作序列便于比较
...
//			System.out.println();
		...
		for(int i = 0;i<3;i++){
			for(int k = 0;k<3;k++){
				...
				for(int m=0;m<3;m++){
					...
					T[i].executeTransation(DB, i);   //对事务i按顺序对x1~x2元素进行读写操作，这里对读写操作进行了封锁?
					T[k].executeTransation(DB, k);	//对事务k按顺序对x1~x2元素进行读写操作，这里对读写操作进行了封锁?
					T[m].executeTransation(DB, m);	//对事务m按顺序对x1~x2元素进行读写操作，这里对读写操作进行了封锁?
	...
	public void concurrencyScheduleWithoutControl(){
		...
		int actionNum = 0;
		while(actionNum!=-6){   //actionNum=-6表示有事务的读写操作都进行完了
			actionNum = 0;
			...
			if(Math.abs(random.nextInt())%2 == Transation.READ && readIndex[whichTransation]!=-1){ //如果是读操作且此事务的读操作还没有进行完
				...
				}												////如果是写操作且此事务的读操作还没有进行完 且（read操作已完成 或对此元素read操作先于write操作，保证write写时的数据不为空�?
...
//							System.out.println("T"+(Ti+1)+">>>rLockNum:"+rLockNum+"   ,lockFinishFlag: "+T[Ti].lockFinishFlag);
		...
		if(map.size()>5){
			System.out.println("出现死锁，程序强制退出！");
			System.exit(0);
		}else{
			System.out.println(" -------->>> Final DB[]: "+Arrays.toString(DB));
		}
